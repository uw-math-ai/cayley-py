...
def seed_everything(seed: int, deterministic: bool = False):
    """Seed Python, NumPy, and PyTorch RNGs for paired-trial reproducibility."""
    seed = int(seed)
    random.seed(seed)
    np.random.seed(seed)
...
CFG['bfs_anchor_pretrain_epochs'] = 100
CFG['bfs_anchor_pretrain_batches_per_epoch'] = 128
...
# pipeline, so results can be compared within the same random-seed family.
CFG['base_seed'] = 20260522
...
# =============================================================================
N_TRIALS = 10
_all_baseline_path_lengths = []
_all_baseline_found = []
_all_baseline_spearman = []
_all_baseline_argmin = []
_all_baseline_mae = []
...
for trial_idx in range(N_TRIALS):
    trial_seed = int(CFG['base_seed']) + int(trial_idx)
    setup_seed = trial_seed
    model_seed = trial_seed + 100_000
    warmup_seed = trial_seed + 200_000
    mdqn_seed = trial_seed + 300_000
    bfs_anchor_seed = trial_seed + 400_000
    residual_seed = trial_seed + 500_000
    beamaware_seed = trial_seed + 600_000
    seed_everything(setup_seed, deterministic=bool(CFG.get('deterministic_torch', False)))
    ...
    run_config["setup_seed"] = setup_seed
    run_config["model_seed"] = model_seed
    run_config["warmup_seed"] = warmup_seed
    run_config["mdqn_seed"] = mdqn_seed
    run_config["bfs_anchor_seed"] = bfs_anchor_seed
    run_config["residual_seed"] = residual_seed
    run_config["beamaware_seed"] = beamaware_seed
    ...
    wandb.log({
        "trial_idx": trial_idx,
        "seed/trial_seed": trial_seed,
        "seed/setup_seed": setup_seed,
        "seed/model_seed": model_seed,
        "seed/warmup_seed": warmup_seed,
        "seed/mdqn_seed": mdqn_seed,
        "seed/bfs_anchor_seed": bfs_anchor_seed,
        "seed/residual_seed": residual_seed,
        "seed/beamaware_seed": beamaware_seed,
        "setup/time_sec": setup_time,
    })

    # -------------------------------------------------------------------------
    # Paired reproduced baseline: standard mDQN + beam search, no BFS anchor,
    # no residual rollout, no beam-aware correction. This is the paper-style
    # pass-1 baseline evaluated under the same trial seed family as our method.
    # -------------------------------------------------------------------------
    print("\n--- Paired Baseline: Standard mDQN only ---")
    seed_everything(model_seed, deterministic=bool(CFG.get('deterministic_torch', False)))
    baseline_model = Net(input_size=n, hidden_dims=CFG['list_layers_sizes'], num_classes_for_one_hot=n).to(device)
    baseline_optimizer = optim.Adam(baseline_model.parameters(), lr=CFG['lr'])

    seed_everything(warmup_seed, deterministic=bool(CFG.get('deterministic_torch', False)))
    baseline_warmup_losses, baseline_warmup_time = train_warmup(
        baseline_model, baseline_optimizer, criterion, list_generators, state_destination,
        vec_hasher, dtype, device, CFG, trial_idx
    )

    seed_everything(mdqn_seed, deterministic=bool(CFG.get('deterministic_torch', False)))
    baseline_losses, baseline_train_time = train_mdqn_pass1(
        baseline_model, baseline_optimizer, criterion, list_generators, tensor_generators,
    ...
    # -------------------------------------------------------------------------
    # Method branch starts from the same initialization/warmup seeds, then adds
    # BFS exact anchoring, residual rollout, and beam-aware search.
    # -------------------------------------------------------------------------
    print("\n--- Method Branch: BFS-anchor + residual + beam-aware ---")
    seed_everything(model_seed, deterministic=bool(CFG.get('deterministic_torch', False)))
    ...
    # -------------------------------------------------------------------------
    # Evaluate the paired standard mDQN baseline on the same start state and
    # the same diagnostic BFS set. This gives the clean paper-style comparison.
    # -------------------------------------------------------------------------
    baseline_path_len, baseline_found, baseline_beam_time = run_beam_search(
        "baseline_standard_mdqn", baseline_model, state_start, state_destination, list_generators,
        tensor_generators, vec_hasher, dtype, device, CFG, trial_idx
    )
    baseline_diag = run_error_analysis(
        "baseline_standard_mdqn", baseline_model, array_states_all_saved, list_distances_save,
        tensor_generators, vec_hasher, device, trial_idx
    )
    _all_baseline_path_lengths.append(int(baseline_path_len))
    _all_baseline_found.append(bool(baseline_found))
    _all_baseline_spearman.append(baseline_diag["spearman_all"])
    _all_baseline_argmin.append(baseline_diag["argmin_match"])
    _all_baseline_mae.append(baseline_diag["mae"])
    wandb.log({
        "trial_idx": trial_idx,
        "trial/baseline_standard_mdqn_path_length": int(baseline_path_len),
        "trial/baseline_standard_mdqn_found": bool(baseline_found),
        "trial/baseline_standard_mdqn_beam_time_sec": float(baseline_beam_time),
        "trial/baseline_standard_mdqn_spearman": baseline_diag["spearman_all"],
        "trial/baseline_standard_mdqn_argmin_match": baseline_diag["argmin_match"],
        "trial/baseline_standard_mdqn_mae": baseline_diag["mae"],
    })
    ...
    seed_everything(bfs_anchor_seed, deterministic=bool(CFG.get('deterministic_torch', False)))
    ...
    if CFG['pass2_enabled']:
        seed_everything(residual_seed, deterministic=bool(CFG.get('deterministic_torch', False)))
        ...
        seed_everything(beamaware_seed, deterministic=bool(CFG.get('deterministic_torch', False)))
        ...
        improvement_baseline_to_bfs_pass1 = int(baseline_path_len) - int(pass1_path_len)
        improvement_baseline_to_residual = int(baseline_path_len) - int(residual_path_len)
        improvement_baseline_to_pass3_best = int(baseline_path_len) - int(best_pass3_path_len)
        ...
        best_of_four_path = min(int(baseline_path_len), int(pass1_path_len), int(residual_path_len), int(best_pass3_path_len))
        ...
        wandb.log({
            ...
            **pass3_sweep_results,
            ...
            "trial/improvement_baseline_minus_bfs_pass1": improvement_baseline_to_bfs_pass1,
            "trial/improvement_baseline_minus_residual": improvement_baseline_to_residual,
            "trial/improvement_baseline_minus_pass3_best": improvement_baseline_to_pass3_best,
            ...
            "trial/best_of_four_path_length": best_of_four_path,
            "trial/baseline_standard_mdqn_found": bool(baseline_found),
        ...
        _trial_rows.append({
            ...
            "mdqn_seed": int(mdqn_seed),
            "bfs_anchor_seed": int(bfs_anchor_seed),
            "residual_seed": int(residual_seed),
            "beamaware_seed": int(beamaware_seed),
            "baseline_standard_mdqn_path_length": int(baseline_path_len),
            "baseline_standard_mdqn_found": bool(baseline_found),
            ...
            "improvement_baseline_minus_bfs_pass1": improvement_baseline_to_bfs_pass1,
            "improvement_baseline_minus_residual": improvement_baseline_to_residual,
            "improvement_baseline_minus_pass3_best": improvement_baseline_to_pass3_best,
            ...
            "best_of_four_path_length": best_of_four_path,
            "baseline_standard_mdqn_spearman": baseline_diag["spearman_all"],
            "baseline_standard_mdqn_mae": baseline_diag["mae"],
            "baseline_standard_mdqn_argmin_match": baseline_diag["argmin_match"],
...
# =============================================================================
summary = {
    "summary/base_seed": int(CFG.get('base_seed', -1)),
    "summary/n_trials": int(N_TRIALS),
    "summary/baseline_standard_mdqn_mean_path_length": float(np.mean(_all_baseline_path_lengths)) if _all_baseline_path_lengths else float('nan'),
    ...
    "summary/baseline_standard_mdqn_found_rate": float(np.mean(_all_baseline_found)) if _all_baseline_found else float('nan'),
    ...
    "summary/baseline_standard_mdqn_mean_spearman": float(np.nanmean(_all_baseline_spearman)) if _all_baseline_spearman else float('nan'),
    ...
    "summary/baseline_standard_mdqn_mean_argmin_match": float(np.nanmean(_all_baseline_argmin)) if _all_baseline_argmin else float('nan'),
    "summary/baseline_standard_mdqn_mean_mae": float(np.nanmean(_all_baseline_mae)) if _all_baseline_mae else float('nan'),
...
if _trial_rows:
    ...
    best4_paths = [row["best_of_four_path_length"] for row in _trial_rows]
    ...
    summary["summary/best_of_four_mean_path_length"] = float(np.mean(best4_paths))
    summary["summary/best_of_four_found_rate"] = float(np.mean([p < CFG['n_steps_limit'] for p in best4_paths]))

    # Distribution summaries. Treat n_steps_limit as the failure sentinel.
    for prefix, vals in [
        ("baseline_standard_mdqn", _all_baseline_path_lengths),
